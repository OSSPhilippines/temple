...
export function startServer(options?: LSOptions) {
  ...
  let templePlugin: TemplePlugin = undefined as any;
  ...
  connection.onInitialize((evt) => {
    ...
    Logger.setDebug(
      (evt.initializationOptions?.configuration?.temple ||
    ...
    configManager.update(
      evt.initializationOptions?.configuration?.temple?.plugin ||
    ...
    pluginHost.register((templePlugin = new TemplePlugin(configManager)));
  ...
  function notifyTsServiceExceedSizeLimit() {
    connection?.sendNotification(ShowMessageNotification.type, {
      message:
        'Temple language server detected a large amount of JS/Temple files. ' +
        'To enable project-wide JavaScript/TypeScript language features for Temple files, ' +
  ...
  connection.onDidChangeConfiguration(({ settings }) => {
    configManager.update(settings.temple?.plugin);
    ...
    Logger.setDebug(settings.temple?.['language-server']?.debug);
  });
  ...
  connection.onRequest('$/getCompiledCode', async (uri: DocumentUri) => {
    ...
    if (doc) {
      const compiled = await templePlugin.getCompiledResult(doc);
      if (compiled) {
